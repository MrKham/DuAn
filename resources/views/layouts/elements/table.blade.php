...
<table id="{{ $special_id }}" class="table table-striped table-bordered" width="100%" style="overflow-x: auto;">
    <thead>
        ...
        <tr>
        	@foreach ($columns as $column)
        		<th style="{{@$column['css']}}">{{ $column['title'] }}</th>
        	@endforeach
        </tr>
    </thead>
...
//		searching: false,
        ...
        pageLength: {{ $pageLength or 25 }},
        order: {!! $orderColumn or "[[0, 'desc']]" !!},
        oLanguage: {
			// sSearch: '<span class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>',
			{{--@if (App::getLocale() != 'en')
	...
	$('.dt-toolbar').remove();
	$('.dt-toolbar-footer').addClass('row');
	@if (isset($reloadTime))
    ...
    // Apply the filter
    $("#{{ $special_id }} thead th input[type=text]").on('change', function () {
